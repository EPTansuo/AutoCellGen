...
foreach cell $cells {
  ...
  if {[string match "2BDFFHQN_*" $cell]} {
    ...
    #define_bus -from 0 -to 1 $cell D 
    #define_bus -from 0 -to 1 $cell QN
    #set_attribute -cells $cell -pins QN0 function "\"IQN\""
    #set_attribute -cells $cell -pins QN1 function "\"IQN\""
  }
  ...
  if {[string match "TT*" $cell]} {
    if {[ALAPI_active_cell $cell]} {
      set info_file "${RUN_DIR}/cell_info/${cell}.info"
      if {![file exists $info_file]} {
        puts "WARNING: No info file found for custom cell ${cell}; skipping define_cell"
      } else {
        set info_in [open ${info_file} "r"]
        set info_lines [split [read $info_in] "\n"]
        close $info_in

        set input_pins [split [string trim [lindex $info_lines 1]] " "]
        set output_pins [split [string trim [lindex $info_lines 2]] " "]
        set pinlist [concat $input_pins $output_pins]

        if {[llength $input_pins] == 0 || [llength $output_pins] == 0} {
          puts "WARNING: Incomplete pin info for custom cell ${cell}; skipping define_cell"
        } else {
