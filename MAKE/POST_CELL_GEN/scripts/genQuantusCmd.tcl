...
set dp_cpu 4
if {[info exists ::env(QTS_DP_CPU)] && $::env(QTS_DP_CPU) ne ""} {
  set dp_cpu $::env(QTS_DP_CPU)
}
puts $output "distributed_processing -multi_cpu ${dp_cpu}\n\n"
puts $output "process_technology -technology_library_file \"${curr_dir}/../inputs/techlib.defs\" -technology_name ${libname} -temperature ${temp_info}\n\n"
puts $output "output_setup -file_name ${cellname}_${libname}.sp\n\n"

puts $output "input_db -type pvs -run_name ${cellname} -directory_name ${input_dir} -format GDS -design_cell_name ${cellname}\n"
